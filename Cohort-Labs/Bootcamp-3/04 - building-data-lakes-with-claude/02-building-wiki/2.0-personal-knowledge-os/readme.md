...
Your wiki is live and self-updating.

In the next lesson, we'll go one step further — using Claude to query across the wiki and your Snowflake tables together to generate reports, surface trends, and answer questions about your channels and companies without you having to look anything up manually.
...
|---------|-------------------|------------|
| **Skill files** | **How It Works** — *"The skill lives at second-brain/skill/build-wiki.md"* | [Claude Code slash commands](https://docs.anthropic.com/en/docs/claude-code/slash-commands) |
| **MCP Tool Use** | **What's Happening Behind the Scenes** — *"Claude calls the Composio MCP server, which holds your authenticated Snowflake connection"* | [MCP Documentation](https://docs.anthropic.com/en/docs/claude-code/mcp) |
| **Incremental Processing** | **Auto-Update Scheduler** — *"query only rows where the row timestamp is newer than the last_processed_at in the sync log"* | [Claude Code Overview](https://docs.anthropic.com/en/docs/claude-code) |
