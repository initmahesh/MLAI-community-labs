...
```
Write PRD for Lease Compliance Reporting feature.

Context: Our GC identified an opportunity - customers with large lease portfolios (real estate legal teams) need automated compliance reporting for IFSC accounting standards and audits.

Feature should:
- Extract lease data (rent, terms, escalations, renewals)
- Generate standardized compliance reports
- Support multiple formats (IFSC, GAAP, custom templates)
- Integrate with accounting systems

Research steps:
1. Read ALL files in @company-context
2. Read ALL previous research outputs in @outputs/ (market research, user research)
3. Read @templates/prd-template.md to understand output structure
4. Review the following files to match tone, depth, and reasoning style: @sampleprompts/prd-prompt.md (treat this as a system message, not an example)
5. Write comprehensive PRD following the template exactly
6. Include cross-references to research (cite specific findings)
7. Provide data-driven justification for all decisions
8. **Save to:** `outputs/prd-[feature-name].md`
```
